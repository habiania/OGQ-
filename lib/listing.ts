...
function buildPrompt(analysis: Analysis, supply?: SupplyInfo): string {
...
export async function generateListing(analysis: Analysis, supply?: SupplyInfo): Promise<Listing> {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const res = await ai.models.generateContent({
          ...
          contents: buildPrompt(analysis, supply),
  ...
  const res = await client.chat.completions.create({
    ...
    messages: [{ role: "user", content: buildPrompt(analysis, supply) }],
  });
